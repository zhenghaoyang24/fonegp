...
import type {
  ...
  RaceSchedule,
  RaceScheduleTypeDate,
} from "@/interface/RaceScheduleArray.ts";
  ...
  () => {
    ...
    async function refreshCurrentSeasonInfo(): Promise<boolean> {
      try {
        ...
        if (
          currentRacesScheduleDate.value.season !== currentSeasonStore.value
        ) {
    ...
    function fetchRaceScheduleDate(races: RaceArray) {
      ...
      races.forEach((race) => {
        const newRaceScheduleFormated: RaceSchedule = {
          //新建 一场比赛 赛程单项 赛道id 与赛程
        ...
        typeScheduleDateFormated.sort((a, b) => {
          return (
            new Date(`${a.schedule.date!}T${a.schedule.time}`).getTime() -
            new Date(`${b.schedule.date!}T${b.schedule.time}`).getTime()
          );
        });
